...
def domainAssumptions : List DomainAssumption := [
  ...
  { id := "D-007",
    ...
    status := .unvalidated },
  { id := "D-008",
    statement := "Historical tool-record acceptance and the current registry for new tool execution are separate target properties; source origin alone does not determine historical representability.",
    falsifier := "A version-pinned target rejects an otherwise structurally valid historical call because its tool is absent from the current registry.",
    validation := "Probe native history ingestion and resumed model context independently of the registry used for new execution; record actual target constraints.",
    status := .conditional },
  { id := "D-009",
...
def requirements : List RequirementRecord := [
  ...
  { id := .semanticFidelity, title := "Common-semantic fidelity",
    statement := "For accepted conversions, the target's resumed model context shall preserve every shared conversation concept, including prior executions as native tool history. Reversible assistant prose about a tool is not equivalent to a native tool record.",
    rationale := "Syntactically valid output can still be semantically wrong.",
    fitCriterion := "Fixture differentials preserve roles, content, native tool records, and execution state; real-target context inspection and a controlled prior-tool recall probe agree. Parser acceptance and own-import round trips alone do not establish fidelity.",
    priority := .mustPreview },
  { id := .noSilentLoss, title := "No silent loss",
    statement := "Use native target structures for representable facts. Retain genuinely unrepresentable data in out-of-band metadata where possible and disclose limitations; converter carrier prose in assistant dialogue does not fulfill native-history fidelity.",
  ...
  { id := .toolLifecycleFidelity, title := "Tool lifecycle fidelity",
    statement := "Every selected-path tool invocation and recorded result shall remain native target history where representable, preserving order, identity or reversible ID mappings, source name, arguments, output, linkage, execution state, output availability, and error provenance. Source-recorded completion survives pruned output; importing history never reruns tools.",
    ...
    fitCriterion := "Seeded native-history tests cover interleaving, parallelism, duplicate/missing IDs, completed/pruned output, retained summaries, interruptions, unknown state, structured results, and errors. Target model context contains native prior tool activity without converter prose; recorded results remain exact, unavailable-output placeholders are explicit, lifecycle counts reconcile, and real load/resume replays no old action.",
    priority := .mustPreview },
...
def machineSpecification : List InterfaceClause := [
  ...
  { id := "S-013-OUTPUT", mentions := [sourceArtifact, invocation],
    constrains := outputArtifact,
    timeReference := .pastOrPresent,
    behavior := "Emit the complete ordered tool lifecycle as native target history where representable; preserve execution state separately from output availability and keep provenance or fallback data outside dialogue." },
  { id := "S-013-STATUS", mentions := [sourceArtifact, invocation],
    constrains := exitStatus,
    timeReference := .pastOrPresent,
    behavior := "Report genuine target limitations; do not count a reversible prose carrier as fulfillment of native-history fidelity." },
...
def fitObligations : List FitObligation := [
  ...
  { id := "F-029A", requirement := .toolLifecycleFidelity,
    statement := "Legacy archive carriers remain inert and decodable. This archive-compatibility check does not fulfill native tool-history fidelity for representable source executions.",
    evidence := ["E-035"], status := .satisfied },
  { id := "F-029", requirement := .toolLifecycleFidelity,
    statement := "Real targets load and resume recorded past tool activity, including native history records, without reexecuting old actions.",
