...
Loom is the private Lean semantic core for `agent-convert`. It models agent
...
|---|---|---|
| Private Lean core | `spec/loom`; owns parsing, IR, policy, rendering, and file/SQLite actuators | Build the immutable `.lake/release/loom` from the clean private source commit |
| Utils compatibility host | `src/convertToPi.ts`; thin `convert-to-pi` host for Claude, Codex, Google AI Studio, and Hermes to Pi | Test inside `utils`; it requires a discoverable Loom binary and has explicit TS rollback |
...
From `utils/`, this smoke is executable after the build above:

```sh
LOOM=spec/loom/.lake/build/bin/loom
"$LOOM" detect spec/loom/parity/fixtures/claude.jsonl
"$LOOM" inspect claude spec/loom/testdata/claude/control-envelopes.jsonl \
  | sed -n '1,18p'
```
...
```sh
LOOM=spec/loom/.lake/build/bin/loom
"$LOOM" cursor-ide spec/loom/test.vscdb cmp-fix loom \
  /tmp/loom-readme-cursor-ide.loom.json
```

## Runnable conversions

All conversion examples run from `utils/` and provide the complete metadata
...
```sh
LOOM_BIN=spec/loom/.lake/build/bin/loom \
  node_modules/.bin/tsx src/convertToPi.ts --json claude \
  spec/loom/testdata/release/continuation-source.claude.jsonl \
...
"$LOOM" convert loom claude \
  spec/loom/testdata/wire/sidechain-detached.v0.json \
